function drawCal(y,m,weekFirst, container) {
	...
	while (true) {
		if (currentWeek==weekFirst) {
			if (newLine!=null) {
				$(container).append(newLine);
				weeks ++;
			}
			newLine = $("<div class='row'></div>");
			newLine.data("line", weeks);
			currentWeek = weekFirst;
			newLine.css("height", dateHeight);
		...
		if (ccIndex>0 || cc[ccIndex].week==currentWeek) {
			...
			/*
			if (cc[ccIndex].solarFestival!="") {
				newDateDiv.append("<div>" + cc[ccIndex].solarFestival + "</div>");
			} else if (cc[ccIndex].lunarFestival!="") {
				newDateDiv.append("<div>" + cc[ccIndex].lunarFestival + "</div>");
			} else {
				newDateDiv.append("<div>" + cDay(cc[ccIndex].lDay) + "</div>");
			}
			*/
			ccIndex++;
			
			/*
			attachEvent(newDateDiv, function(t) {
				$(t).addClass("date-selected");
				
				if ($("div.upped, dik iv.downed").length>0) {
					$("div.upped").css("top", "+=100");
					$("div.downed").css("top", "-=100");
					$("div.upped, div.downed").removeClass("upped").removeClass("downed");
				}
				$(t).parent().addClass("upped"); 
				$(t).parent().prevAll().addClass("upped");
				$(t).parent().nextAll().addClass("downed");
				$(t).parent().css("top", "-=100");
				$(t).parent().prevAll().css("top", "-=100");
				$(t).parent().nextAll().css("top", "+=100");
			});
			*/
		}
		newLine.append(newDateDiv);
		currentWeek++;
		if (currentWeek==7) currentWeek=0;
		
		if (ccIndex==cc.length) {
			$(container).append(newLine);
			break;
		}
	}
